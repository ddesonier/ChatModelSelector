...
if ($currentAccountJson) {
    try {
        ...
        Write-Host "  ❌ Error parsing account info" -ForegroundColor Red
        exit 1
    }
} else {
...
if ($resourceJson) {
    try {
        ...
        Write-Host "  ❌ Error parsing resource info" -ForegroundColor Red
        exit 1
    }
} else {
...
if ($deploymentsJson) {
    try {
        ...
        Write-Host "  ❌ Error parsing deployment info" -ForegroundColor Red
    }
} else {
